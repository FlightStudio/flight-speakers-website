...
**JWT implementation:**
- Tokens are signed with `JWT_SECRET` (required env var — server won't start without it)
- Stored in httpOnly cookies (not accessible to client-side JavaScript)
- `sameSite: 'strict'` prevents CSRF
- `secure: true` in production forces HTTPS-only
- 24-hour expiration
- Every admin endpoint is guarded by `requireAdmin` middleware

**Password handling:**
- Hashed with bcrypt (12 rounds) — never stored in plaintext
- Login returns generic "Invalid credentials" — doesn't reveal whether username or password was wrong

### 8.2 Input Validation

All user-facing endpoints validate inputs before processing:

| Endpoint | Validation |
|----------|-----------|
| `POST /api/enquiry` | Required fields (name, email, org, brief), email regex, max lengths (brief: 5000, name/org: 200) |
| `POST /api/parse-brief` | Brief required, max 5000 chars |
| `GET /api/search` | Query required, max 2000 chars |
| `POST /api/admin/login` | Username + password required |
| `POST /api/admin/speakers` | Name, headline, photo, bio required |
| `PATCH /api/admin/enquiries/:id` | Status whitelisted to valid enum values |
| `GET /api/admin/speakers/analytics` | Period whitelisted to `['day', 'week', 'all']` |

**SQL injection:** All database queries use parameterized statements (`$1`, `$2`, etc.). No string concatenation in queries.

**Enum whitelisting:** Extracted values from AI parsing (eventType, budgetRange, engagementType) are validated against hardcoded allowlists before use.

### 8.3 CORS & Network Security
...
- Whitelist-based CORS — only configured origins can make requests
- `credentials: true` allows cookie-based auth
- Default restricts to localhost for development

### 8.4 Error Handling

**Public-facing routes** return generic error messages. The global error handler never leaks `err.message` or stack traces to the client:
...
Each route handler catches its own errors and returns safe, specific messages (e.g. "Failed to submit enquiry").

### 8.5 Portal Token Security

Speaker self-service portal tokens use:
- `crypto.randomBytes(32)` — cryptographically random
- 7-day expiration
- Single-use flag — marked as consumed after submission to prevent replay
- Portal link generation uses `APP_URL` env var when available to avoid host header injection

### 8.6 Known Limitations & Recommendations

These are documented trade-offs, not oversights:

| Area | Status | Notes |
|------|--------|-------|
| Rate limiting | Not implemented | Add `express-rate-limit` before production — especially on `/api/search`, `/api/parse-brief`, and `/api/admin/login` |
| Security headers | Not implemented | Add `helmet` middleware for CSP, X-Frame-Options, HSTS, etc. |
| Error boundary | Missing | React app has no top-level ErrorBoundary — a component throw crashes the whole page |
| TypeScript | Not used | Plain JS throughout — runtime type errors are possible |
| `express.json()` body limit | Default (100kb) | Sufficient for current use but should be explicitly configured for production |
| Console logging | Verbose | `console.error` calls throughout — replace with structured logger (e.g. pino) before production |

### 8.7 Code Quality Notes

**Shared constants:** Animation easing is defined once in `src/constants/animation.js` and imported everywhere — keeps motion consistent and DRY.

**Prefetch cache:** `src/utils/prefetch.js` uses a 5-minute TTL to avoid serving stale data indefinitely.

**Server-side caching:** `server/services/claude.js` caches semantic search results for 5 minutes with an LRU-style eviction (max 100 entries).

**Fire-and-forget pattern:** Analytics writes (views, recommendations) use `.catch()` to never block the response. If the DB write fails, the user doesn't notice.

**Graceful degradation:** Search falls through: vector + rerank → all-speakers Claude → full-text search → featured speakers. The user always gets results.
